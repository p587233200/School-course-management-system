...
<div class="max-w-xl mx-auto bg-white p-6 rounded shadow mt-6">
    @php
        $submission = $assignment->submissions->firstWhere('studentID', session('user_id'));
        // dd($submission);
    @endphp
    <h2 class="text-2xl font-bold mb-4">作業成績：{{ $assignment->title }}</h2>

    <div class="mb-4">
        <h3 class="text-xl font-semibold">我的成績</h3>
        <p>我的分數：{{ $submission->score ?? '未上傳作業' }}</p>
    </div>

    <div class="mb-4">
        <h3 class="text-xl font-semibold">我的評語</h3>
        <p>我的評語：{{ $submission->feedback ?? '未上傳作業' }}</p>
    </div>
    ...
    <div class="mb-4">
        <a href="{{ route('student.course.detail', $assignment->courseID) }}"
